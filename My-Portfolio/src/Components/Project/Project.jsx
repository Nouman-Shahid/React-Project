        ...
        <Carousel breakpoints={breakpoints} showArrows={false}>
          {
            carousel.map((item, index) => (
              <div key={index} className="col col-md-9 col-lg-5 mx-3 my-3" data-aos="zoom-out" data-aos-duration="1500" style={{ maxHeight: '90vh' }} >
                <div className="card h-100 dark-card">
                  <video src={item.video} controls className="card-img-top" style={{ minHeight: '10vh' }} />
                  <div className="card-body">
                    <h5 className="card-title">{item.title}</h5>
                    {windowWidth > 560 && <p className="card-text">{item.desc}</p>}
                  </div>
